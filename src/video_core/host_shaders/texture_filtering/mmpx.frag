...
void main()
{
	...
	// Inject a bias (0.0001) to counteract negative precision drift at the left/top boundaries
	vec2 pixel_pos = tex_coord * vec2(tex_size) + vec2(0.0001); 

	// coordinates for JKLM quadrant determination
	vec2 JKLM_pos = fract(pixel_pos);

	// uantize continuous coordinates into discrete integer pixel indices.
	// Clamp the range to [0, tex_size - 1] to prevent texture-space overflow during texelFetch
	ivec2 curr_pixel = min(ivec2(pixel_pos), tex_size - 1);

	// Spatial boundary validation macro
	#define checkp(c, d) ( \
		curr_pixel.x + (c) >= 0 && curr_pixel.x + (c) < tex_size.x && \
		curr_pixel.y + (d) >= 0 && curr_pixel.y + (d) < tex_size.y \
	)

	// gracefully truncates out-of-bounds lookups to transparent
	#define src(x, y) ( \
		checkp(x, y) ? texelFetch(tex, curr_pixel + ivec2(x, y), 0) : vec4(0.0) \
	)

////////////////////////////////////////////////////////////////////////////////
	vec4 E = texelFetch(tex, curr_pixel, 0);
...
if (!skiprest) {
	...
	vec4 PC = src(+1, -2);
	vec4 QA = src(-2, -1);
	vec4 QG = src(-2, +1); //             AA    PA    [P]   PC    CC
	vec4 RC = src(+2, -1); //                ┌──┬──┬──┐
	vec4 RI = src(+2, +1); //             QA │  A │  B │ C  │ RC
	vec4 SG = src(-1, +2); //                ├──┼──┼──┤
	vec4 SI = src(+1, +2); //            [Q] │  D │  E │ F  │ [R]
	vec4 AA = src(-2, -2); //                ├──┼──┼──┤
	vec4 CC = src(+2, -2); //             QG │  G │  H │ I  │ RI
	vec4 GG = src(-2, +2); //                └──┴──┴──┘
	vec4 II = src(+2, +2); //             GG    SG    [S]   SI    II
...
if (!skiprest && !eq_E_F&&eq_E_D&&eq_B_F&&eq_F_H && all_eq2(E,C,I) && (eq(E,Q)||El>Fl) && neq(F,src(+3, 0)) ) {K=admixK(vF,vE); M=K;skiprest=true;}	//* RIGHT
if (!skiprest && !eq_E_D&&eq_E_F&&eq_B_D&&eq_D_H && all_eq2(E,A,G) && (eq(E,R)||El>Dl) && neq(D,src(-3, 0)) ) {J=admixK(vD,vE); L=J;skiprest=true;}	//* LEFT
if (!skiprest && !eq_E_H&&eq_E_B&&eq_D_H&&eq_F_H && all_eq2(E,G,I) && (eq(E,P)||El>Hl) && neq(H,src(0, +3)) ) {L=admixK(vH,vE); M=L;skiprest=true;}	//* BOTTOM
if (!skiprest && !eq_E_B&&eq_E_H&&eq_B_D&&eq_B_F && all_eq2(E,A,C) && (eq(E,S)||El>Bl) && neq(B,src(0, -3)) ) {J=admixK(vB,vE); K=J;}				//* TOP

}
	//final write

	frag_color = (JKLM_pos.x < 0.5) ? (JKLM_pos.y < 0.5 ? J : L) : (JKLM_pos.y < 0.5 ? K : M);
